...
class State {
	...
	int getRoundNumber() { return cardsPlayed.size()/3 + 1; }
	
	// If the Deck still does not have every card played, then the game is still valid
	boolean isGameValid() { return cardsPlayed.size() < 54; }

	// If this Round does not have 3 cards played, then the Round is still going
	boolean validRound() { return currentRound.size() < 3; }
	...
	// check if a valid firstInRound move was made
		if (firstInRound()) { return true; }
		 	else {
	...
	int calculatePoints() {
		int points = 0;
		boolean anyTrolls = this.anyTrolls();
		for (Card c : currentRound) {
	...
	// NOTE: This MUST return an int from 0 to 2! ALWAYS DO % playerOrder.size();
